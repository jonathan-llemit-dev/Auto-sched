    ...
    <nav class="bg-white border-b border-gray-100">
            ...
            <div class="flex justify-between h-16">
                <div class="flex">
                    ...
                    <div class="shrink-0 flex items-center">
                        <Link href="{{ route('prof.dashboard') }}">
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                        <x-nav-link :href="route('prof.dashboard')" :active="request()->routeIs('prof.dashboard')">
                            {{ __('Dashboard') }}
                        </x-nav-link>
                    </div>
                </div>
                    ...
                    <x-dropdown placement="bottom-end">
                        <x-slot name="trigger">
                            <button class="flex items-center text-sm font-medium text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition duration-150 ease-in-out">
                                <div>{{ Auth::guard('prof')->user()->profFName }}</div>
                        ...
                        <x-slot name="content">
                            <x-dropdown-link :href="route('prof.profile.edit')">
                                {{ __('Profile') }}
                            </x-dropdown-link>
                            ...
                            <form method="POST" action="{{ route('prof.logout') }}">
                                ...
                                <x-dropdown-link as="a" :href="route('prof.logout')" onclick="event.preventDefault(); this.closest('form').submit();">
                                    {{ __('Log Out') }}
                                </x-dropdown-link>
                            </form>
                        </x-slot>
        ...
        <div v-bind:class="{ block: toggled, hidden: !toggled }" class="sm:hidden">
            <div class="pt-2 pb-3 space-y-1">
                <x-responsive-nav-link :href="route('prof.dashboard')" :active="request()->routeIs('prof.dashboard')">
                    {{ __('Dashboard') }}
            ...
            <div class="pt-4 pb-1 border-t border-gray-200">
                <div class="px-4">
                    <div class="font-medium text-base text-gray-800">{{ Auth::guard('prof')->user()->profFName }}</div>
                    <div class="font-medium text-sm text-gray-500">{{ Auth::guard('prof')->user()->email }}</div>
                </div>

                <div class="mt-3 space-y-1">
                    <x-responsive-nav-link :href="route('prof.profile.edit')">
                        {{ __('Profile') }}
                    </x-responsive-nav-link>
                    ...
                    <form method="POST" action="{{ route('prof.logout') }}">
                        ...
                        <x-responsive-nav-link as="a" :href="route('prof.logout')" onclick="event.preventDefault(); this.closest('form').submit();">
                            {{ __('Log Out') }}
                        </x-responsive-nav-link>
                    </form>
                </div>
